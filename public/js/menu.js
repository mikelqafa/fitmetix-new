(function ($) {
  ...
  let MaterialMenu = function () {
    ...
    MaterialMenu.prototype.init = function (config) {
      ...
      let data = this.$menu.data(DATA_KEY)
    ...
    MaterialMenu.prototype.handleItemClick_ = function (e) {
      ...
      let $item = this.$menuContainer.find('.' + this.Classes_.ITEM + '.'+this.Classes_.SELECTED)
      if($item.length){
        $item.attr('selected', '')
        $item.removeClass(this.Classes_.SELECTED)
      }
      $(e.currentTarget).addClass(this.Classes_.SELECTED).attr('selected', 'selected')
      if(data !== undefined && data !== null)
        this.$menu.trigger('selected', {option: option, value: data})
      window.setTimeout(function () {
        this.hide()
      }.bind(this), 100)
    }

    MaterialMenu.prototype.handleClickForBackDrop = function () {
      this.hide()
    }
    ...
    MaterialMenu.prototype.show = function () {
      ...
      if (this.config.location === 'auto') {
        this.setAutoPosition_(this.config.position)
      }else {
    ...
    MaterialMenu.prototype.reset = function () {
      let $item = this.$menuContainer.find('.' + this.Classes_.ITEM + '.'+this.Classes_.SELECTED)
      if($item.length){
        $item.attr('selected', '')
        $item.removeClass(this.Classes_.SELECTED)
      ...
      switch (p) {
        case 'bottom left':
          this.$menu.css('transform-origin', 'left bottom 0').css({
            'left': rect.left,
            'top': position.bottom - rect.height,
            'right': 'auto',
            'bottom': 'auto'
          })
          break
